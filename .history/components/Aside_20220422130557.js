...
export const Aside = ({ toggleStyle }) => {
  ...
  return (
    <nav
      className={`h-screen bg-white  absolute top-0 bottom-0 left-0  shadow-lg overflow-hidden w-3/4 -translate-x-full  ${toggleStyle}`}
    >
      <div className="top  ">
        <div className="top flex  justify-between items-center py-2 px-2">
          <p className="logo"> logo</p>
          <GrFormClose onClick={() => setOpenedAside(!OpenedA)} />
        </div>
        <Divider />
        <h4 className="text-xs text-red-500  pl-5 py-1 "> Account</h4>
        {AsideLink.map((item) => (
          <div key={item.id} className="flex items-center px-2 py-2">
            <span className="mr-2 hover:text-red-400" onClick={ (setOpenedAside) => (!OpenedAside)}>{item.icon}</span>
            <h1> {item.title}</h1>
          </div>
        ))}
      </div>
